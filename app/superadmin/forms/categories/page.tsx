...
export default function FormCategoriesPage() {
  ...
  const handleDelete = async (categoryId: string) => {
    ...
    try {
      const response = await fetch(`/api/forms/categories/${categoryId}`, {
        method: 'DELETE',
      });
  ...
  if (loading) return <div className="flex justify-center p-4"><span className="loading loading-spinner loading-lg"></span></div>;
  if (error) return <div className="p-4 text-red-500">{error}</div>;

  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-8">Form Categories Management</h1>
